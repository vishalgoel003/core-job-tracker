...
- **[EXEC-4.5]** Terminal Execution Fallback: If a terminal command fails due to permission denial or sandbox restrictions, do NOT repeatedly request permissions. Instead, immediately present the exact command to the user and ask them to execute it in their local terminal (with venv activated) and paste the output back. Format the command in a fenced code block for easy copy-paste. Continue your work based on the user-provided output.